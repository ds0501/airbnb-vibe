...
interface MyHomesClientProps {
  homes: SerializedHome[];
  currentUser: User;
  favoritedHomeIds: string[];
}

export default function MyHomesClient({ homes, currentUser, favoritedHomeIds }: MyHomesClientProps) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8">
        {homes.map((home) => (
          <ListingCard
            ...
            isFavorited={favoritedHomeIds.includes(home.id)}
          />
        ))}
      </div>
    </div>
